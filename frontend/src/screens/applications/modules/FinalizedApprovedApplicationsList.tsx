...
const FinalizedApprovedApplicationsList = () => {
  ...
  const [selectedApplication, setSelectedApplication] = useState<number | null>(null);  // Allow only one selection
  const [page, setPage] = useState<number>(1);
  const [limit, setLimit] = useState<number>(10);
const [selectedApplications, setSelectedApplications] = useState<any>([]);

const handleCheckboxChange = (application: FinalizedApprovedApplication) => {
  ...
  // Handle radio button change (only one selection at a time)
  const handleRadioChange = (id: number) => {
    setSelectedApplication(id);  // Set the selected application id
  };

  // Handle Approve Citation button click
  const handleApproveCitation = async () => {
    if (!selectedApplication) {
      toast.error("Please select one application to approve.");
      return;
    }
    ...
    try {
      ...
      const restPayload = {
        type: "citation",  // Application type
        status: "approved", // Status of the application (approved in this case)
        member: {
          name: "Sudhanshu",
          ic_number: "123456",
          member_type: "member_officer",  // Member's type (e.g., officer, etc.)
          member_id: "e4adb597-f6c6-4f4d-92f4-a6444015193c",  // Unique member ID
          is_signature_added: true,  // Whether the signature is added
          sign_digest: "something while developing",  // Placeholder for signature digest
        },
        level: "division",  // Level of the application (e.g., division, unit, etc.)
        // citation_issued: true,  // Flag indicating if the citation is issued
      };

      // Call the API to approve the citation for the selected application
      const response = await Axios.put(
        `${apiEndPoints.citation}/${selectedApplication}`,  // API endpoint with application ID
        restPayload
      );

      if (response.data.success) {
        toast.success("Citation approved successfully");
        setSelectedApplication(null);  // Clear selection after approval
      } else {
        toast.error(response.data.message ?? "Failed to approve citation");
      }
    } catch (error:any) {
      toast.error(
        error.response?.data?.message ?? "Error approving citation"
      );
    }
  };
