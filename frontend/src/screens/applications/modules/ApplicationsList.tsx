...
<button
  ...
  onClick={async () => {
    try {
      const applicationsForFinalized = selectedUnits.map((unit) => ({
        id: unit.id,
        type: unit.type,
      }));
  ...
  {loading ? (
    ...
    units.map((unit: any) => {
      ...
      console.log("Unit Debug =>", {
        id: unit.id,
        last_approved_by_role: unit?.last_approved_by_role,
        status_flag: unit?.status_flag,
        is_mo_approved: unit?.is_mo_approved,
        is_ol_approved: unit?.is_ol_approved,
        canShowCheckbox,
      });
